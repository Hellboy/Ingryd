...
class InstitucionModel
{
	...
	public function Listar()
	{
		try
		{
			...
			$stm = $this->pdo->prepare("
				select *
				from  entidad e inner join institucion i inner join sucursal s
				where i.institucionID = e.entidadID AND e.estado = 1 and e.entidadID= s.entidadID");
			$stm->execute();
	...
	public function Obtener($id)
	{
		try 
		{
			$stm = $this->pdo
			          ->prepare("select *
				from  entidad e inner join institucion i inner join sucursal s
				where i.institucionID = e.entidadID and e.entidadID= s.entidadID and s.entidadID = ?");
	...
	public function Registrar(entidad_institucion $data)
	{
		...
		$sql = "INSERT INTO entidad(nombreInstitucion , descripcion , encargado , direccion, ciudad, departamento, latitud , longitud, horario , estado) VALUES (?, ?,  ?, ?, ?,?, ?,  ?, ?, ?)";
		 ...
		 $sql2= "INSERT INTO sucursal (sucursalID, entidadID) VALUES (?,?)";

		$this->pdo->prepare($sql and $sql2 and $sql3)
		     ->execute(
			array(
		...
		try 
		{
			$del = $this->pdo
			          ->prepare("UPDATE entidad SET 
						...
						estado        = 0
				    WHERE sucursalID =  entidadID and entidadID= institucionID and  entidadID = ?");			          
